package net.hex_wards.casting.patterns.spells
...
import net.hex_wards.networking.SetLookPitchS2CMsg
...
class OpCongrats : SpellAction {
    ...
    override fun execute(args: List<Iota>, ctx: CastingContext): Triple<RenderedSpell, Int, List<ParticleSpray>> {
        ...
        if (target !is ServerPlayer) throw MishapBadEntity(
            target,
            Component.translatable("text.hex_wards.congrats.player")
        )
    ...
    private data class Spell(val player: ServerPlayer) : RenderedSpell {
        override fun cast(ctx: CastingContext) {
            player.sendSystemMessage(Component.translatable("text.hex_wards.congrats", player.displayName));
            Hex_wardsNetworking.sendToPlayer(player, SetLookPitchS2CMsg(-90f))
        }
    }
}
